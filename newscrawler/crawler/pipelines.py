...
class DatabaseStorage(object):
    ...
    def __init__(self):
        ...
        self.insertCurrent = ("INSERT INTO CurrentVersion(localPath,\
                              ...
                              hashedName,title,ancestor)VALUES(%(localPath)s,\
                              %(modifiedDate)s, %(downloadDate)s,\
                              %(sourceDomain)s, %(url)s, %(hashedName)s,\
                              %(title)s, %(ancestor)s)")

        self.insertArchive = ("INSERT INTO ArchiveVersion(id,localPath,\
                              modifiedDate, downloadDate, sourceDomain, url,\
                              hashedName, title, ancestor)VALUES( %(id)s,\
                              %(localPath)s, %(modifiedDate)s,\
                              %(downloadDate)s, %(sourceDomain)s, %(url)s,\
                              %(hashedName)s, %(title)s, %(ancestor)s)")
        ...
        def process_item(self, item, spider):
                ...
                try:
                        temp = self.cursor.execute(self.compareVersions,
                                                   (item['url'],))
